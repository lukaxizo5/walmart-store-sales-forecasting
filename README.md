...
Missingness ძირითადად დროებითია: მონაცემები მხოლოდ 2011 წლის
...
| **52-კვირიანი lookback** (26-ის ნაცვლად) | ~1,890 → ~2,961 |
...
1. **Patching**: ჩვეულებრივ, ტრანსფორმერი თითო კვირას ცალკე ტოკენად იღებდა,
   რაც გრძელი სერიისთვის ძვირი და არაეფექტურია. PatchTST ამის ნაცვლად **მიმდევრობით კვირებს
   აჯგუფებს **patch-ებად** ( 13 კვირის ბლოკები, ყოველ 4 კვირაში).
   თითოეული patch ხდება ერთი ტოკენი. სარგებელი: (ა) patch იჭერს **ლოკალურ პატერნს** (მაგ. ერთი
   თვის ფორმას), (ბ) ტოკენების რაოდენობა მცირდება, ამიტომ მოდელს შეუძლია **უფრო გრძელი ისტორიის**
   დამუშავება ზედმეტი ძალისხმევის გარეშე.
2. **Channel-independence**: თითო სერია მუშავდება **ცალკე, მაგრამ ერთი და იმავე ტრანსფორმერის წონებით** — სერიებს შორის attention არ ხდება. ეს ხდის მოდელს გლობალურსა და მდგრადს.
...
2. **Patching** — ნორმალიზებული ფანჯარა იჭრება გადამფარავ 13-კვირიან patch-ებად (თითო patch — ერთი
   ტოკენი).
3. **Self-attention** — ტრანსფორმერის მექანიზმი, სადაც თითო patch „უყურებს" ყველა დანარჩენს და
   წყვეტს, რომელი რამდენად მნიშვნელოვანია მისთვის. 
4. **Linear head** — ბოლო წრფივი ფენა, რომელიც attention-ით დამუშავებულ patch-ებს გარდაქმნის
   საბოლოო პროგნოზად (მომდევნო 13 კვირა).
5. **დენორმალიზაცია (RevIN-ის შებრუნება)**
...
- **masked patch inputs**: ტრენინგისას ზოგიერთი patch შემთხვევით „იფარება", რაც რეგულარიზაციის მექანიზმია. 
...
- **Holdout WMAE: 1706.6** — **საუკეთესო holdout WMAE ყველა მოდელს შორის**
...
| მოდელი | Forecast strategy | CV WMAE | Holdout WMAE |
|---|---|---|---|
| **TimesFM** (zero-shot) | foundation | — | **1706.6** |
...
სერიაზე** (56.8%). ორი მოდელი განსხვავებულ ცდომილებებს უშვებს, ამიტომ მათი **50/50 გაერთიანება** სავარაუდოდ
შეამცირებს საერთო ცდომილებას — ეს არის soft-voting ensemble მოდელის კლასიკური ლოგიკა.
...
holdout WMAE-ს (~1707). ეს მოსალოდნელია და გულწრფელად უნდა აღვნიშნოთ — holdout არის **in-sample
ვალიდაცია** (2012-02 → 2012-10), ხოლო Kaggle test არის **მომავალი პერიოდი** (2012-11 → 2013-07),
რომელიც მოიცავს სრულ სადღესასწაულო და მომავალ პერიოდს.
...
**მეტრიკის სახელები** შენარჩუნებულია იდენტურად ყველა არქიტექტურაზე (case-sensitive), რაც wandb/MLflow-ს
საშუალებას აძლევს, ერთ chart-ზე დაადოს სხვადასხვა მოდელის შედეგი.
...
4. **გულწრფელი ვალიდაცია** — ორი scope-ის (representative vs full) მკაფიო გამიჯვნა, in-sample
   holdout-სა და Kaggle future test-ს შორის სხვაობის აღიარება და leakage-safe feature-ების
